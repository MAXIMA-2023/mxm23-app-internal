...
const BottomBarMenu = ({ title, activeIcon, inactiveIcon, isActive, placement, children }: { title: string; activeIcon: As; inactiveIcon: As; isActive: boolean; placement: PlacementWithLogical; children: ReactNode }) => {
  return (
    <Menu autoSelect={false} offset={[0, 16]} placement={placement}>
      <MenuButton as={Button} rounded={"full"} variant={"ghost"} cursor={"pointer"} h={"4em"}>
    ...
    <>
      ...
      >
        <Flex
          ...
          color={"gray.700"}
        >
          {(auth.role === "panit" && auth.user?.divisiID === "D01") || (auth.role === "panit" && auth.user?.divisiID === "D02") ? (
            <>
              <BottomBarMenu title={""} activeIcon={MdAdminPanelSettings} inactiveIcon={MdOutlineAdminPanelSettings} isActive={pathname === "/dashboard/verifikasi" || pathname === "/dashboard/toggles"} placement="top-start">
          ...
          {(auth.role === "panit" && auth.user?.divisiID === "D01") || (auth.role === "panit" && auth.user?.divisiID === "D02") || (auth.role === "panit" && auth.user?.divisiID === "D10") ? (
            <>
              <Menu autoSelect={false} offset={[0, 32]} placement="top">
                {({ isOpen }) => (
                  <>
                    <MenuButton as={Button} rounded={"full"} variant={"link"} cursor={"pointer"} h={"4em"}>
          ...
          {auth.role === "panit" || auth.role === "organisator" ? (
            <>
              <BottomBarMenu title={""} activeIcon={MdWorkspacesFilled} inactiveIcon={MdOutlineWorkspaces} isActive={pathname.includes("/dashboard/state") || pathname.includes("/malpun/")} placement="top-end">
                <Accordion allowToggle>
                  <AccordionItem border={"none"} cursor={"pointer"} transition={"0.1s ease-in-out"}>
                    ...
                    <AccordionPanel ml={"1.5em"} p={0}>
                      ...
                      {auth.role === "organisator" ? (
                        <>
                          <Stack as={Link} href={"/dashboard/state/details"} direction={"row"} align={"center"} my={"0.5em"} color={pathname === "/dashboard/state/details" ? activeColor : inactiveColor}>
                            <Icon as={pathname === "/dashboard/state/details" ? MdPeople : MdOutlinePeople} />
